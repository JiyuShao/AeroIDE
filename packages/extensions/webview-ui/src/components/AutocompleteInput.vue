...
const versionSuggestions = computed(() => {
  return versions.value
    .filter(
      (item) =>
        !config.value.excludeVersions.some((exclusion) =>
...
const addPackage = async ({ dev }: { dev: boolean } = { dev: false }) => {
  ...
  await withUpdate(typings?.name, async () => {
    if (!typings) return;
    if (await API.getIsPackageDeprecated(typings.name)) return;
    const isInstalled = store.state.installedPackages.some(
...
const handleMessage = async (message: MessageEvent) => {
  if (message.data?.type === "FOCUS_INPUT") {
...
onMounted(() => {
  ...
  vscode.postMessage({ type: "AUTOCOMPLETE_INPUT_MOUNTED" });
});
...
watch(packageName, async (value) => {
  ...
  try {
    ...
    const suggestions = await API.getSuggestions(
      value,
      config.value.maxNumberOfResults
    );
...
<template>
  <div
    :class="{
      'flex-auto':
        !store.state.filterInputIsFocused && !store.state.filterQuery,
    }"
  >
    <OnClickOutside @trigger="hideAutoComplete">
      <div class="autocomplete">
        ...
        <div
          v-if="state !== State.Inactive && query.length"
          class="results codesandbox:border-[color:var(--vscode-sideBySideEditor-horizontalBorder)] codesandbox:bg-[color:var(--vscode-sideBar-background)]"
          @click.stop
        >
          <div
          ...
          <div class="results__content" ref="resultsRef">
            ...
            >
              <div class="results__item-header">
                <div>
                  <span
                    class="results__item-title [&_em]:text-[color:var(--vscode-textPreformat-foreground)]"
	...
	c21.3,0.2,35.2-13.6,35.2-42.7c0.2-25.4-12.5-38.8-32.7-38.8c-5.2,0-8.6,0.5-10.6,1.1V211.2z"
          ...
          <div
            v-if="
              ...
              (config.showAlgoliaInfo || config.showShortcuts)
            "
            class="results__shortcuts"
          >
            <hr
              v-if="config.showShortcuts"
              class="px-3 mb-0.5 border-t opacity-50 border-[color:var(--vscode-focusBorder)] codesandbox:border-[color:var(--vscode-sideBySideEditor-horizontalBorder)]"
            />
            <div v-if="config.showShortcuts" class="results__shortcut px-3">
            ...
            <hr
              class="mt-0.5 border-t opacity-50 border-[color:var(--vscode-focusBorder)] codesandbox:border-[color:var(--vscode-sideBySideEditor-horizontalBorder)]"
            />
            <div
              v-if="config.showAlgoliaInfo"
              class="results__algolia pb-2 px-3"
            >
...
.results__item--active .results__item-description {
  color: inherit;
}
