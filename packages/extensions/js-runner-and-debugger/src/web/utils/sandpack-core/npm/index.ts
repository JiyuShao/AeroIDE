...
export async function getDependenciesFromSources(
  ...
  try {
      ...
      Object.keys(prebundledDependencies).map(depName =>
        getPrebundledDependency(depName, depsWithNodeLibs[depName])
          .then(d => {
            ...
            // dynamically.
            // @ts-expect-error not possible anymore
            if (d.error) {
    ...
    const [dynamicLoadedDependencies, prebundledLoadedDependencies] =
...
export async function loadDependencies(
  ...
  if (Object.keys(dependencies).length !== 0) {
    ...
    const dependenciesWithoutTypings = pickBy<Record<string, any>>(
      dependencies,
      (_val, key) => !(key.includes && key.includes('@types'))
    );
