...
import { getAbsoluteDependency } from '../../common/utils/dependencies';
    ...
    .then(async response => {
      if (!response.ok) {
        ...
        try {
          ...
          logger.error('callApi response.text() error:', err);
        }
...
async function requestPackager(
  ...
  logger.debug(`Trying to call packager for ${retries} time`);
